var TabManager = React.createFactory(React.createClass({
  ...
  render:function(){
    ...
    return React.DOM.div({className:"outer"}, React.DOM.div({className:"window container"},
      ...
      React.DOM.div({className:"window searchbox"},
        ...
        React.DOM.div({className:"icon windowaction duplicates",title:"Select Duplicates",onClick:this.showDup}),
        React.DOM.div({className:"icon windowaction selectall",title:"Select All",onClick:this.selectAll}),
  ...
  deleteTabs:function(){
    ...
    if(tabs.length){
      chrome.runtime.sendMessage({type:'delete', tabs:tabs});
    }else{
      /*
  ...
  selectAll:function(e){
    ...
    {
      for (key in this.state.tabsbyid)
      {
        if (this.state.hiddenTabs[key] && this.state.filterTabs)
        {
          // Ignore.
          delete this.state.selection[key]; // Make sure not selected
        }
        else
        {
          if (this.state.selection[key])
            delete this.state.selection[key];
          else
            this.state.selection[key] = true;
        }
      }
      this.forceUpdate();
      return;
    ...
    for (key in this.state.tabsbyid)
    {
      ...
      {
        // Don't count this.
        this.state.selection[key] = false; // Make sure not selected
      }
      else
      {
        all_selected = all_selected && this.state.selection[key];
      }
    }
    for (key in this.state.tabsbyid)
    {
  ...
  showDup:function(){
    ...
    for (key in this.state.tabsbyid)
    {
      var tab = this.state.tabsbyid[key];
      if (!(tab.url in urls)) urls[tab.url] = [];
      urls[tab.url].push(tab);
      this.state.hiddenCount++;
      this.state.hiddenTabs[key] = true;
    }
    for (url in urls)
    {
      var values = urls[url];
      ...
      for (var v in values)
      {
        ...
        {
          this.state.selection[v.id] = true;
        }
        delete this.state.hiddenTabs[v.id];
        this.state.hiddenCount--;
      }
      //break;
    }
