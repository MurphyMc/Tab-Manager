var Window = React.createFactory(React.createClass({
  ...
  bookmark:function(e){
    ...
    tabs = tabs.map( tab => {
      //console.log(tab);
      return {url:tab.props.tab.url, title:tab.props.tab.title, id:tab.props.tab.id};
    });
    ...
    chrome.runtime.sendMessage({type:'bookmark', tabs:tabs, default_name:d + " " + t, close:e.nativeEvent.shiftKey});
  },
  close:function(){
